DECLARE @UsersData nvarchar(MAX) = 
...
--select ISJSON(@UsersData) As Valid

Select 
P2.UserID
,P1.Active
,DATEADD(S,CONVERT(INT,LEFT(P3.CreatedDate,10)),'1970-01-01 00:00:00') As CreatedDate
,DATEADD(S,CONVERT(INT,LEFT(P4.LastLogin,10)),'1970-01-01 00:00:00') As LastLogin
,P1.Role
,P1.Source
,P1.State
INTO dbo.USERS
From OPENJSON(@UsersData)
WITH (
	ID NVARCHAR(MAX) '$._id' AS JSON,
	Active BIT '$.active',
	CreateDate NVARCHAR(MAX) '$.createdDate' AS JSON,
	[Login] NVARCHAR(MAX) '$.lastLogin' AS JSON,
	[Role] NVARCHAR(25) '$.role',
	Source NVARCHAR(10) '$.signUpSource',
	State NVARCHAR(2) '$.state'
) AS P1
CROSS APPLY OPENJSON(P1.ID)
WITH (
	UserID NVARCHAR(100) '$."$oid"'
) AS P2
CROSS APPLY OPENJSON(P1.CreateDate)
WITH (
	CreatedDate NVARCHAR(15) '$."$date"'
) AS P3
OUTER APPLY OPENJSON(P1.Login)
WITH (
	LastLogin NVARCHAR(15) '$."$date"'
) AS P4
